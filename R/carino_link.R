...
carino_link <- function(total_returns, components) {
  ...
  k <- dplyr::if_else(abs(total_returns) < 1e-10, 1.0, log(1 + total_returns) / total_returns)
  ...
  if (is.null(dim(cum_weighted_comp))) {
    cum_weighted_comp <- matrix(cum_weighted_comp, nrow = n, ncol = ncol(comp_mat))
  }
  cum_weighted_total <- cumsum(k * total_returns)

  safe_total <- dplyr::if_else(abs(cum_weighted_total) < 1e-12, NA_real_, cum_weighted_total)
  cum_components <- cum_weighted_comp * (cum_geometric / safe_total)
  cum_components[is.na(cum_components)] <- 0

  tibble::as_tibble(
    stats::setNames(as.data.frame(cum_components), paste0("cumulative_", colnames(components)))
  ) %>%
