...
constrained_linear_regression <- function(
  ...
  if (!is.matrix(x) || !is.numeric(x)) {
    stop(paste0(
      "`x` must be a numeric matrix; got object of class ",
  ...
  if (!is.numeric(y) || is.matrix(y) || is.data.frame(y)) {
    stop(paste0(
      "`y` must be a numeric vector; got object of class ",
  ...
  if (!non_negative && !sum_to_one) {
    betas <- base::solve(
      base::crossprod(x_design),
      base::crossprod(x_design, y)
    )
  ...
  constraints <- construct_linear_regression_constraints(
    k_predictors,
    non_negative,
    sum_to_one
  )
  ...
  if (intercept) {
    ...
    if (nrow(constraints$Amat) > 0) {
      constraints$Amat <- cbind(
        rep(0, nrow(constraints$Amat)),
        constraints$Amat
      )
    } else {
