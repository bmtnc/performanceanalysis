...
roll_constrained_lm <- function(x, y, width, ...) {
    ...
    function(window_data) {
      ...
      if (nrow(window_data) > width) {
        window_data <- window_data[
          (nrow(window_data) - width + 1):nrow(window_data),
        ]
      }
  ...
  if (n_incomplete > 0) {
    warning(paste0(
      "First ",
      n_incomplete,
      " observations have NA coefficients due to insufficient window size (width = ",
      width,
      ")."
    ))
  }
  ...
  structure(
    list(coefficients = coef_matrix, width = width, call = match.call()),
