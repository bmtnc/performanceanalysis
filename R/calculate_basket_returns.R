...
calculate_basket_returns <- function(return_data, basket_tickers) {
  ...
  if (!is.character(basket_tickers)) {
    stop(paste0(
      "basket_tickers must be a character vector, got: ",
      class(basket_tickers)[1]
    ))
  }
  ...
  if (length(missing_tickers) > 0) {
    stop(paste0(
      "The following basket_tickers are not found in return_data: ",
  ...
  if (nrow(result) == 0) {
    stop(paste0(
      "No dates found with complete return data for all ",
      length(basket_tickers),
      " basket tickers"
    ))
  }
