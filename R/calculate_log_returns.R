...
calculate_log_returns <- function(
  ...
  if (!is.character(ticker_col) || length(ticker_col) != 1) {
    stop(paste0(
      "Input 'ticker_col' must be a single character string. Received: ",
      class(ticker_col)[1],
      " of length ",
      length(ticker_col)
    ))
  ...
  if (!is.character(price_col) || length(price_col) != 1) {
    stop(paste0(
      "Input 'price_col' must be a single character string. Received: ",
      class(price_col)[1],
      " of length ",
      length(price_col)
    ))
  ...
  if (!is.character(return_col) || length(return_col) != 1) {
    stop(paste0(
      "Input 'return_col' must be a single character string. Received: ",
      class(return_col)[1],
      " of length ",
      length(return_col)
    ))
