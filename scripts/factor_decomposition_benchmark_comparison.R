...
message(paste0(
  "Total observations in factor_differences: ",
  nrow(factor_differences)
))
message(paste0(
  "Date range: ",
  min(factor_differences$date),
  " to ",
  max(factor_differences$date)
))
...
if (nrow(range_violations) > 0) {
  message(paste0(
    "\nWARNING: ",
    nrow(range_violations),
    " dates have factor differences outside [-1, 1]"
  ))
